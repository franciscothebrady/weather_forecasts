...
# historical_freq <- read_csv("data/historical_freqs.csv")
...
historical_freq <- station_obs %>%
  ...
  group_by(prcp.id, prcp.prcp) %>%
  summarise(freq = n())

#write_csv(historical_freq, "weather_forecasts/data/historical_freqs.csv")
# counting total number of rain event days per year per station
historical_freq <- historical_freq %>%
  group_by(prcp.id) %>%
  mutate(total_prcp_events = sum(freq)) %>%
  group_by(prcp.id, prcp.prcp) %>%
  mutate(total_prcp_cat_count = sum(freq))
# divide number of rain events in each prcp category over total
# to get a "probability of historical precipitation at that precp category"
historical_freq <- historical_freq %>% 
  mutate(prcp_prob = total_prcp_cat_count / total_prcp_events) %>%
  select(-year, -freq) %>%
  distinct()
# merge into events df Q24 forecast
events1 <- left_join(events, historical_freq, by = c("GHCND.ID" = "prcp.id", "prcp.prcp"))

# write to csv so we can merge in sheldus if(exists())
