...
storm_events <- inner_join(storm_events, cbsa, by = c("STATE" = "State.full", 
                                                      "CZ_NAME" = "County.name", 
...
freq_map <- bdry_map + 
  geom_polygon(data = cbsa_map_freq_f, aes(long, lat, group = group),
               fill = cbsa_map_freq_f$event_freq)
freq_map
