...
## 2018-4-17, reorganized some code. added old code section [fb]
...
# some cleaning was done here, but now it's in 1-storm_events_cleaning.R and
...
temp_df <- data.frame(id = events$EVENTS.ID[!is.na(events$EVENTS.begin_lat)],
                      ...
                      longitude = events$EVENTS.begin_lon[!is.na(events$EVENTS.begin_lat)])
...
# Merge MET stations into events
...
#-- merge station_obs with events by ID and date
...
# rename using rename 
events <- rename(events, GHCND.prcp_cat = prcp.prcp)

#### USE get_archived_GFSX_MOS.R instead! ####
# this is where the old mesonet mos function was.
  ...
  if (is.null(mos_df)) {
    ...
    mos5day12 <- rbind(mos5day12,
                       cbind.data.frame(
                         ...
                         P12=dplyr::filter(mos_df, FCDT==as.Date(events$EVENTS.begin_date[eid], tz = "Zulu"))$P12)
    )
  }
}
...
for (eid in 1:length(events$EVENTS.ID)) {
  print(eid)
  mos_df <- get_archived_GFSX_MOS(events$MOS.ICAO[eid],
                                  format(events$EVENTS.begin_date[eid]-1, "%Y%m%d"),
                                  "00Z")
  if (is.null(mos_df)) {
...
#-- merge forecast data to events ID and date
rm(ghcnd_station_list)
###
#### merge in Q12 forecasts with sheldus events ####
mos_q12 <- merge(mos1day12, mos5day12, by.x="index", by.y="index")
mos_q12 <- data.frame(Q12.f1=mos_q12$Q12.x, 
                      Q12.f5=mos_q12$Q12.y,
                      P12.f1=mos_q12$P12.x,
                      P12.f5=mos_q12$P12.y)
...
mos_q24 <- data.frame(Q24.f2=mos_q24$Q24.x,
                      Q24.f6=mos_q24$Q24.y,
...
#### END OF OLD STORM CLEANING ####
#### OLD MESONET MOS FUNCTION ####
